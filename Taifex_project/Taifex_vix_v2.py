...
def get_vix_data_from_server(data_selection_id):

    response = None

    try:
        if data_selection_id == 1:
            # Full vix time series
            response = requests.post('https://mis.taifex.com.tw/futures/api/getChartDataTick', headers=headers, cookies=cookies, data=data)
        
        elif data_selection_id == 2:
            response = requests.post('https://mis.taifex.com.tw/futures/api/getChartData1M_mem', headers=headers, cookies=cookies, data=data)
        
        else:
            raise Exception("Wrong seleciton id")
    ...
    if response.status_code == requests.codes.ok:
        ...
        if data_selection_id == 1:
            # Full vix time series
            output_to_csv(vix_ticks, file_name='vix-data-full.csv')
        else:
            # Last vix time series
            output_to_csv(vix_ticks, file_name='vix-data-last.csv')
...
def main():
    ...
    msg_list = ["Enter action number", "1. Get full vix data from Server", "2. Get last vix data from Server", "3. Draw from local csv file","> "]
    msg = '\n'.join(msg_list)

    action_ID = int( input(msg) )

    if action_ID == 1:
        get_vix_data_from_server(data_selection_id=1)

    elif action_ID == 2:
        get_vix_data_from_server(data_selection_id=2)
